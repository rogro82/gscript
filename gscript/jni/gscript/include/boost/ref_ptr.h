...
private:
	...
	friend long ref_ptr_get_count(RefCountable* p);
	// Boost functions required by ref_ptr.
	friend void ref_ptr_add_ref(RefCountable* p);
	friend void ref_ptr_release(RefCountable* p);
};

inline long ref_ptr_get_count(RefCountable* p) {

	return p->_ref_count;
}

inline void ref_ptr_add_ref(RefCountable* p) {

	__sync_add_and_fetch(&p->_ref_count, 1);
}

inline void ref_ptr_release(RefCountable* p) {
